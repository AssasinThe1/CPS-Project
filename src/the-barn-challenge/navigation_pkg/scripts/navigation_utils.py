...
def cost_trajectory(currentPos, projectedPos, target_sector, candidate_heading, prev_heading=None, d_free=4.0, max_range=4.0, mode='orig'):
    """
    ...
    if prev_heading is not None:
        switching_penalty = abs(candidate_heading - prev_heading)
         
    if mode == 'new':
        d_free_capped = min(d_free, max_range)
        free_space_penalty = max_range - d_free_capped
        W1 = 0.1
        W2 = 0.2
        W3 = 0.5
    else:
        free_space_penalty = 0.0 # Ignore free space
        W1 = 0.1
        W2 = 0.2
        W3 = 0.0
        
    total_cost = progress_cost + (W1 * heading_deviation) + (W2 * switching_penalty) + (W3 * free_space_penalty)
...
def vfh_star_full(currentPos, currentHeading, heading_sector, ds, ng, hb, threshold, robotDim, wideValleyMin, prev_heading, h, mode):
    """
    ...
    max_range = 4.0 # Make sure this matches your ROS node
    ...
    while open_list:
        ...
        for cand in candidates:
            sector_idx = int(cand) % len(h) 
            
            danger_score = h[sector_idx]
            d_free = max_range - danger_score
            
            proj_pos, proj_heading = project_trajectory(node.position, node.heading, cand, ds)
            cost = cost_trajectory(node.position, proj_pos, heading_sector, cand, prev_heading, d_free, max_range, mode)
