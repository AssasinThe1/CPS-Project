...
def cleanup_zombies():
    print("\n[Cleanup] Forcibly killing ROS and Gazebo processes...")
    # Suppress output so it doesn't clutter your terminal
    subprocess.run(['pkill', '-9', '-f', 'ros'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    subprocess.run(['pkill', '-9', '-f', 'gzserver'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    subprocess.run(['pkill', '-9', '-f', 'gzclient'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--out2', type=str, default="benchmark_results.csv")
    parser.add_argument('--heuristic', type=str, default='new', choices=['orig', 'new'])
    ...
    rospy.set_param('/vfh_heuristic', args.heuristic)
    rospy.set_param('/world_idx', args.world_idx)
    rospy.set_param('/run_num', args.run)
    ...
    launch_file = './navigation_pkg/launch/VFH_launch.launch'
    ...
    while compute_distance(goal_coor, curr_coor) > 1 and not collided and curr_time - start_time < TIMEOUT:
        curr_time = rospy.get_time()
        pos = gazebo_sim.get_model_state().pose.position
        curr_coor = (pos.x, pos.y)
 ...
 ##########################################################################################
    ...
    with open(args.out, "a") as f:
        f.write("%d %d %d %d %.4f %.4f\n" %(args.world_idx, success, collided, (curr_time - start_time)>=TIMEOUT, curr_time - start_time, nav_metric))
    with open(args.out2, "a") as f:
        f.write(f"{args.world_idx},{args.heuristic},{success},{collided},{(curr_time - start_time)>=TIMEOUT},{curr_time - start_time:.4f},{nav_metric:.4f}\n")
